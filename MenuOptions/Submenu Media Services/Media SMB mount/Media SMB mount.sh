...
EOF
  ...
  tee <<-EOF >> /etc/fstab
#media
//$storage_server/$fstorage_share /data cifs noperm,rw,credentials=/root/.storage_credentials,uid=1000,gid=1000,file_mode=0660,dir_mode=0770,vers=3.0 0 0
EOF
  mount -a
}
...
finalization () {
  ...
  msgbox "All Done!"
  echo
  echo "Your media mount is now available under /data"
  echo

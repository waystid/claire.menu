...
sudo mkdir -p /config/Traefik
tee <<-EOF >/config/.traefik.env
...
volumes="    volumes:
      - /config/\${APP_NAME:?err}:/etc/traefik
...
sudo touch /config/Traefik/acme.json
sudo chmod 600 /config/Traefik/acme.json

# # Create the config files

sudo tee <<-EOF >/config/Traefik/traefik.yml
global:
...
sudo tee <<-EOF >/config/Traefik/fileConfig.yml
http:
